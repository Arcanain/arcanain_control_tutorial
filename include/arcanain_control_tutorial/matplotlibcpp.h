...
private:
      ...
      _interpreter()
      {
        ...
        if (!s_backend.empty()) {
          PyObject_CallMethod(
            matplotlib, const_cast < char * > ("use"), const_cast < char * > ("s"),
            s_backend.c_str());
        }
...
// a non-numpy alternative for `detail::get_2darray()`.
  ...
  void plot_surface(
    ...
    const std::map < std::string, std::string > & keywords = std::map < std::string,
    std::string > (),
    ...
    const std::map < std::string, std::string > & keywords)
  {
    ...
    PyObject * res =
      PyObject_Call(detail::_interpreter::get().s_python_function_stem, args, kwargs);
  ...
  template < typename Numeric >
  bool boxplot(
    const std::vector < Numeric > & data, const std::map < std::string,
    std::string > & keywords = {})
  {
    detail::_interpreter::get();
  ...
  bool bar(
    ...
    std::string ls = "-", double lw = 1.0, const std::map < std::string,
    std::string > & keywords = {})
  {
    detail::_interpreter::get();
  ...
  template < typename Numeric >
  bool bar(
    const std::vector < Numeric > & y, std::string ec = "black", std::string ls = "-",
    double lw = 1.0,
    const std::map < std::string, std::string > & keywords = {})
  {
  ...
  template <
  typename NumericX, typename NumericY, typename NumericZ, typename NumericU, typename NumericW,
  typename NumericV >
  bool quiver(
    const std::vector < NumericX > & x, const std::vector < NumericY > & y,
    const std::vector < NumericZ > & z,
    ...
    const std::map < std::string, std::string > & keywords = {})
  {
    ...
    assert(
      x.size() == y.size() && x.size() == u.size() && u.size() == w.size() && x.size() == z.size() &&
  ...
  template < typename NumericX, typename NumericY >
  bool named_plot(
    const std::string & name, const std::vector < NumericX > & x,
  ...
  inline void save(const std::string & filename, const int dpi = 0)
  {
    ...
    PyObject * res =
      PyObject_Call(detail::_interpreter::get().s_python_function_save, args, kwargs);
    if (!res) {
